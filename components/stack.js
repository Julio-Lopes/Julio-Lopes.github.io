const categorias = [
  {
    nome: 'frontend', icon: 'ti-device-desktop', cor: '#a78bfa',
    skills: ['JavaScript', 'TypeScript', 'React', 'Next.js', 'HTML5', 'CSS3', 'SCSS', 'Bootstrap'],
  },
  {
    nome: 'sfcc', icon: 'ti-shopping-cart', cor: '#c084fc',
    skills: ['SFRA', 'ISML', 'Page Designer', 'Business Manager', 'Cartridges'],
  },
  {
    nome: 'backend', icon: 'ti-server-2', cor: '#5dcaa5',
    skills: ['Node.js', 'NestJS', 'Java', 'Spring Boot', 'PHP (CodeIgniter)', 'APIs REST'],
  },
  {
    nome: 'banco de dados', icon: 'ti-database', cor: '#7c9fe0',
    skills: ['PostgreSQL', 'MySQL', 'MongoDB'],
  },
  {
    nome: 'cloud / devops', icon: 'ti-cloud', cor: '#e0a878',
    skills: ['Oracle Cloud (OCI)', 'Docker', 'Git'],
  },
  {
    nome: 'ferramentas', icon: 'ti-tools', cor: '#d4537e',
      ...
      <div class="stack__grid">
        ${categorias.map(cat => `
          <article class="stack__card" style="border-top-color:${cat.cor}">
            ...
            <div class="stack__pills">
              ${cat.skills.map(s => `<span class="stack__pill">${s}</span>`).join('')}
            </div>
          </article>
        `).join('')}
      </div>
